...
@section('content')
    <form role="form" autocomplete="off" method="post"  action="{{route('admin.article.update-article')}}">
        {{csrf_field()}}
        <input type="hidden" value="{{$article->id}}" name="id">
        <div class="form-group">
            <label>Title</label>
            <input class="form-control" placeholder="Title" name="title" value="{{ old('title',$article->title) }}"/>
            <div class="text-danger">{{ $errors->post->first('title') }}</div>
        </div>

        <div class="form-group">
            <label>Questions</label>
            <textarea class="mceEditor"  name="questions">{{ old('questions',$article->questions) }}</textarea>
        ...
        <div class="form-group">
            <label>Answers</label>
            <textarea class="mceEditor" id="text"  name="answers" >{{ old('answers',$article->answers) }}</textarea>
